...
def load_repository(repo_path):
    global current_repo, chat_history, last_user_query
    ...
    # 🔥 2. CLEAR OLD PERSISTED FILES
    import shutil

    if os.path.exists("faiss_index"):
        shutil.rmtree("faiss_index")

    if os.path.exists("repo_files.json"):
        os.remove("repo_files.json")
    ...
    # 🔥 3. BUILD NEW VECTOR STORE
    save_path = main(repo_path)
    ...
    # 🔥 4. BUILD BM25 USING ACTUAL TEXT
    texts = [m["text"] for m in metadata]
    bm25 = build_bm25(texts)

    chunks = texts  # compatibility
    ...
    print("FINAL FILES:", len(files))

    # 🔥 6. SAVE FILE LIST
    with open("repo_files.json", "w") as f:
        json.dump(files, f)

    # 🔥 7. BUILD GRAPH (OPTIONAL)
    ...
    # 🔥 8. STORE FINAL STATE (FIXED: update instead of reassignment)
   # 🔥 FINAL ATOMIC UPDATE (ONLY ONCE)
    ...
    current_repo.clear()
    current_repo.update(new_repo)

    print("Repo stored in memory:", len(current_repo.get("files", [])))
# -------------------------------
...
def stream_answer(query):
    ...
    # 🔥 FILE-LEVEL RETRIEVAL FIX
    if ".py" in query or ".js" in query or ".java" in query:
        file_matches = [m for m in metadata if query.lower() in m["file"].lower()]
        ...
        if file_matches:
            ...
            retrieved_files = [m["file"] for m in file_matches]
    ...
    # -------- RAG SEARCH --------
    if not goto_llm:
        results = hybrid_search(query, index, metadata, bm25)
    ...
    if not context_chunks:
        # 🔥 fallback: use raw metadata text
        context_chunks = [
            (m["text"], m["file"])
            for m in metadata[:5]
        ]
        retrieved_files = [m["file"] for m in metadata[:5]]
...
async def upload_github(data: dict):
    ...
    try:
        ...
        if not zip_data:
            return {"error": "Could not download repo (invalid URL or branch)"}

        # Save zip
        zip_path = os.path.join(path, "repo.zip")
        with open(zip_path, "wb") as f:
            f.write(zip_data)
